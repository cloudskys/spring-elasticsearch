...
public class EsProductServiceImpl implements EsProductService {
	...
	@Override
	public Page<EsProduct> searchHighLightNew(String keyword, Integer pageNum, Integer pageSize) {
		 BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
		  PageRequest pageRequest = PageRequest.of(0,10);
	        NativeSearchQueryBuilder queryBuilder = new NativeSearchQueryBuilder()
	                .withQuery(boolQuery)
	                .withHighlightFields(
	                        new HighlightBuilder.Field("productCategoryName").preTags("<span style=\"color:red\">").postTags("</span>"),
	                        new HighlightBuilder.Field("subTitle").preTags("<span style=\"color:red\">").postTags("</span>"))
	                .withPageable(pageRequest);
	        //String queryString = query.getQueryString();
	        // 最佳字段  + 降低除了name之外字段的权重系数
	        MatchQueryBuilder nameQuery = QueryBuilders.matchQuery("productCategoryName", "T恤");
	        MatchQueryBuilder authorQuery = QueryBuilders.matchQuery("subTitle", "2018夏季新品短袖T恤男HNTBJ2E080A 蓝灰花纹80 175/92A/L80A 蓝灰花纹80").boost(0.8f);
        ...
        Page<EsProduct> search = elasticsearchTemplate.queryForPage(searchQuery,EsProduct.class, extResultMapper);
	    return search;
	}
